...
namespace El_Khalil
{
    public partial class The_InSafe : Form
    {
        ...
        private void ReturnData()
        {
            if (radioButton1.Checked)
            {
                using (dataSet = Ezzat.GetDataSet("_Safe_InTransaction_Day", "X", new SqlParameter("@Day", DateTime.Parse(dateTimePicker3.Value.ToString()))))
                {
            ...
            {
                using (dataSet = Ezzat.GetDataSet("_Safe_InTransaction_During", "X",
            ...
            {
                using (dataSet = Ezzat.GetDataSet("_Safe_InTransaction_All", "X"))
                {
                    dataGridView1.DataSource = dataSet.Tables["X"];
                }
            }
